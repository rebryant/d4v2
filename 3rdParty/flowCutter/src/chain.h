...
typename std::enable_if<
    is_id_id_func<L>::value && is_id_func<R>::value && !is_id_id_func<R>::value,
...
chain(const L& l, const R& r) {
  ...
  for (int i = 0; i < l.preimage_count(); ++i) result[i] = r(l(i));
  return result;  // NVRO
}
...
chain(L l, const R& r) {
  ...
  for (int i = 0; i < l.preimage_count(); ++i) l.set(i, r(l(i)));
  l.set_image_count(r.image_count());
  return std::move(l);
}

template <class L, class R>
typename std::enable_if<is_id_id_func<L>::value &&
