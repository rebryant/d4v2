...
class ArrayIDFunc {
 public:
  ...
  explicit ArrayIDFunc(int preimage_count) : preimage_count_(preimage_count) {
  ...
  ArrayIDFunc(const IDFunc& o) : preimage_count_(o.preimage_count()) {
    if (preimage_count_ == 0)
      data_ = nullptr;
    else {
      data_ = new T[preimage_count_];
      try {
        for (int id = 0; id < preimage_count_; ++id) data_[id] = o(id);
      } catch (...) {
        delete[] data_;
        throw;
      }
  ...
  ArrayIDFunc(const ArrayIDFunc& o) : preimage_count_(o.preimage_count_) {
    if (preimage_count_ == 0)
      data_ = nullptr;
    else {
      data_ = new T[preimage_count_];
  ...
  typename std::enable_if<
      is_id_func<IDFunc>::value &&
          std::is_convertible<typename id_func_image_type<IDFunc>::type,
                              T>::value,
      ArrayIDFunc&>::type
