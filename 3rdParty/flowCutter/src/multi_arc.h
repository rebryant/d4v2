...
bool is_symmetric(const Tail& tail, const Head& head) {
  ...
  for (int i = 0; i < arc_count; ++i) {
    if (tail(forward_arc_list(i)) != head(backward_arc_list(i)) ||
        head(forward_arc_list(i)) != tail(backward_arc_list(i)))
      return false;
  }
  return true;
}
...
bool has_multi_arcs(const Tail& tail, const Head& head) {
  return count_true(identify_non_multi_arcs(tail, head)) !=
         tail.preimage_count();
}
