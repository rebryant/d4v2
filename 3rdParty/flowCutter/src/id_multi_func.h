...
struct ArrayIDMultiFunc {
  ...
  Range<T*> operator()(int id) {
    assert(0 <= id && id < preimage_count() && "id out of bounds");
    return {intermediate_to_image.begin() +
                *std::begin(preimage_to_intermediate(id)),
            intermediate_to_image.begin() +
                *std::end(preimage_to_intermediate(id))};
  }
...
ArrayIDIDMultiFunc invert_id_id_func(const IDIDFunc& f) {
  ...
  for (int i = 0; i < f.preimage_count(); ++i) ++begin[f(i)];
