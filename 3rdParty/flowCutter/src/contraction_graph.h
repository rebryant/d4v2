...
class EdgeContractionGraph {
 public:
  ...
  void forall_nodes_in_last_computed_neighborhood(const F& f) {
    for (int i = 0; i < neighborhood_size; ++i) f(neighborhood(i));
  }
  ...
  void compute_neighborhood_of(int v) {
    ...
    if (union_find_parent[v] == v) {
      ...
      do {
        // Iterate over the adjacency
        {
          ...
          while (arc_in_begin != arc_in_end) {
            // Compress union find path
            {
              ...
              while (union_find_parent[x] != x) x = union_find_parent[x];
            ...
            assert(union_find_parent[arc_head[arc_in_begin]] ==
                   arc_head[arc_in_begin]);

            // Only keep the nodes that are not the heads of loops or multi arcs
            if (!in_neighborhood(arc_head[arc_in_begin]) &&
                arc_head[arc_in_begin] != v) {
        ...
        // Goto next non-empty adjacency in the ring, and rewire the ring
        // pointer to skip them in future
        int next_adjacency = next_adjacency_in_ring[current_adjacency];
        while (out_arc_begin[next_adjacency] == out_arc_end[next_adjacency] &&
               next_adjacency != initial_adjacency)
...
class NodeContractionGraph {
 public:
  ...
  void forall_neighbors_then_contract_node(int v, const F& callback) {
    ...
    g.forall_nodes_in_last_computed_neighborhood([&](int u) {
      if (is_virtual(u)) g.rewire_arcs_from_second_to_first(v, u);
    });
...
int compute_chordal_supergraph(const Tail& tail, const Head& head,
  ...
  for (int x = 0; x < tail.image_count() - 1; ++x) {
    int upward_degree = 0;
    g.forall_neighbors_then_contract_node(x, [&](int y) {
      on_new_arc(x, y);
      ++upward_degree;
    });
...
public:
        ...
        SimpleNodeContractionAdjGraph(int node_count, const ArcList&arc_list):
                g(node_count, arc_list), is_virtual(node_count, false),
in_neighborhood(node_count, false){
        }
                        ...
                        for(auto u:g.out(v)){
                                if(!is_virtual[u.target]){
                                        if(!in_neighborhood[u.target]){
                                                in_neighborhood[u.target] =
true; neighborhood.push_back(u);
                                        }
                                }else{
                                        for(auto w:g.out(u.target)){
                                                ...
                                                if(w.target != v){
                                                        if(!in_neighborhood[w.target]){
                                                                in_neighborhood[w.target]
= true; neighborhood.push_back(w);
                                                        }
                                                }
                                        }
                                }
...
std::vector<Arc>compute_simple_contraction_hierarchy(const NodeList&node_list,
const ArcList&arc_list){ SimpleNodeContractionAdjGraph g(range_size(node_list),
arc_list); std::vector<Arc>shortcuts; for(int v:node_list){ for(auto
u:g.out_followed_by_contract_node(v)) shortcuts.push_back({v, u.target});
        }
