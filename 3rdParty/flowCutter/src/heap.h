...
class kway_min_id_heap {
 public:
  ...
  void fill(const Range& range) {
    ...
    for (auto r : range) {
      assert(0 <= r.first && r.first < (int)id_pos.size() &&
             "range must contain valid id");
  ...
  void clear() {
    ...
    for (int i = 0; i < heap_end; ++i) contained_flags[heap[i].id] = false;
    heap_end = 0;

#ifdef INTERNAL_HEAP_CHECKS
    for (auto x : contained_flags) assert(!x);
#endif
 ...
 private:
  ...
  void rebuild_heap() {
    for (int i = heap_end - 1; i >= 0; --i) move_down(i);
  }
  ...
  void move_down(int pos) {
    ...
    for (;;) {
      ...
      for (int i = begin + 1; i < end; ++i) {
        if (order(heap[i].key, heap[min_child_pos].key)) min_child_pos = i;
      }

      if (!order(heap[min_child_pos].key, key)) break;
