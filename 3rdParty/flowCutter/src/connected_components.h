...
ArrayIDIDFunc compute_connected_components(const Tail& tail, const Head& head) {
  ...
  auto is_representative =
      id_func(node_count, [&](int i) { return uf.is_representative(i); });

  return chain(uf, compute_keep_function(is_representative,
                                         count_true(is_representative)));
}
...
template <class OutArc, class Head, class OnRootFirstVisit,
          class OnRootLastVisit, class OnTreeUpArcVisit,
          class OnTreeDownArcVisit, class OnNonTreeArcVisit>
void symmetric_depth_first_search(
  ...
  for (int i = 0; i < node_count; ++i) next_out[i] = std::begin(out_arc(i));
...
template <class OutArc, class Head, class BackArc>
ArrayIDIDFunc compute_biconnected_components(const OutArc& out_arc,
                                             const Head& head,
  ...
  auto on_first_root_visit = [&](int x) { depth[x] = 0; };
  ...
  symmetric_depth_first_search(out_arc, head, on_first_root_visit,
                               on_last_root_visit, on_tree_down_arc_visit,
                               on_tree_up_arc_visit, on_non_tree_arc_visit);

#ifndef NDEBUG
  for (int i = 0; i < arc_count; ++i) assert(arc_component[i] != -1);
#endif
...
bool is_biconnected(const Tail& tail, const Head& head) {
  return compute_biconnected_components(
             invert_id_id_func(tail), head,
             compute_back_arc_permutation(tail, head))
             .image_count() <= 1;
}
}  // namespace flowCutter
