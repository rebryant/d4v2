...
void print_tree_decompostion_of_order(std::ostream& out, ArrayIDIDFunc tail,
  ...
  out << "s td " << nodes_in_bag.size() << ' ' << maximum_bag_size << ' '
      << node_count << '\n';
  for (int i = 0; i < bag_count; ++i) {
    out << "b " << (i + 1);
    for (auto x : nodes_in_bag[i]) out << ' ' << (order(x) + 1);
    out << '\n';
  }
...
void print_tree_decompostion_of_multilevel_partition(
  ...
  out << "s td " << bag_count << ' '
      << get_treewidth_of_multilevel_partition(cell_list) << ' '
      << get_node_count_of_multilevel_partition(cell_list) << '\n';
