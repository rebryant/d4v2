...
struct TinyIntIDFunc {
 private:
  ...
  static constexpr std::uint64_t entry_mask =
      (std::uint64_t(1) << bit_count) - std::uint64_t(1);
 ...
 public:
  ...
  explicit TinyIntIDFunc(int preimage)
      : preimage_(preimage),
        data_((preimage + entry_count_per_uint64 - 1) /
              entry_count_per_uint64) {}
  ...
  TinyIntIDFunc(
      ...
      : preimage_(other.preimage_count()),
        data_((other.preimage_count() + entry_count_per_uint64 - 1) /
              entry_count_per_uint64) {
    for (int i = 0; i < preimage_count(); ++i) set(i, other(i));
  }
  ...
  void set(int id, std::uint64_t value) {
    ...
    data_[index] ^=
        ((((data_[index] >> offset) & entry_mask) ^ value) << offset);
  }
  ...
  template <class IDFunc>
  TinyIntIDFunc operator=(
      const typename std::enable_if<is_id_func<IDFunc>::value, IDFunc>::type&
          other) {
...
inline BitIDFunc& operator^=(BitIDFunc& l, const BitIDFunc& r) {
  ...
  for (int i = 0; i < l.data_.preimage_count(); ++i) l.data_[i] ^= r.data_[i];
  return l;
}
