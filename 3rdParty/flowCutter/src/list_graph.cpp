...
static ListGraph load_pace_graph_impl(std::istream& in) {
  ...
  while (std::getline(in, line)) {
    ...
    if (!was_header_read) {
      ...
      int h, t;
      if (!(lin >> t >> h))
        throw std::runtime_error("Can not parse line num " +
                                 std::to_string(line_num) + " \"" + line +
                                 "\" in pace file.");
      --h;
      --t;
      if (h < 0 || h >= graph.node_count() || t < 0 || t >= graph.node_count())
        throw std::runtime_error("Invalid arc in line num " +
                                 std::to_string(line_num) + " \"" + line +
                                 "\" in pace file.");
  ...
  if (next_arc != graph.arc_count())
    throw std::runtime_error(
        "The arc count in the header (" + std::to_string(graph.arc_count()) +
