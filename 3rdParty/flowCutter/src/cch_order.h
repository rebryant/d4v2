...
inline bool is_valid_partial_order(const ArrayIDIDFunc& partial_order) {
...
ArrayIDIDFunc compute_path_graph_order(int node_count,
...
ArrayIDIDFunc compute_tree_graph_order(const Tail& tail, const Head& head,
  ...
  for (int xy = 0; xy < arc_count; ++xy) ++deg[tail(xy)];
  ...
  for (auto& x : order) x = input_node_id(x);
...
ArrayIDIDFunc compute_tree_graph_order(const Tail& tail, const Head& head) {
  return compute_tree_graph_order(
      tail, head, id_id_func(tail.image_count(), tail.image_count(), [](int x) {
        return x;
      }));
}

// Computes an optimal order for a trivial graph. If the input graph is not
// trivial, then the task is forwarded to the compute_non_trivial_graph_order
// functor parameter. A graph is trivial if it is a clique or a tree.
//
...
ArrayIDIDFunc compute_trivial_graph_order_if_graph_is_trivial(
  ...
  bool is_clique = (static_cast<long long>(node_count) *
                        static_cast<long long>(node_count - 1) ==
                    static_cast<long long>(arc_count)),
  ...
  if (is_clique || has_no_arcs) {
    order = id_id_func(node_count, input_node_id.image_count(),
                       [&](int x) { return input_node_id(x); });
  } else if (is_tree) {
    order = compute_tree_graph_order(std::move(tail), std::move(head),
                                     std::move(input_node_id));
  } else {
    order = compute_non_trivial_graph_order(std::move(tail), std::move(head),
                                            std::move(input_node_id));
  }
...
// This function internally reorders the nodes in preorder, then recurses on
// each component of the graph. should_place_node_at_the_end_of_the_order is
// called with the id of some node of the component and the function should
// decide whether this component is placed at the end of the order or at the
// front. If the relative component order does not matter, then let
// should_place_node_at_the_end_of_the_order always return false.
//
// compute_connected_graph_order should order the nodes in each component. The
// order should map node IDs in the graph that is given to input node IDs.
template <class ComputeConnectedGraphOrder,
          class ShouldPlaceNodeAtTheEndOfTheOrder>
ArrayIDIDFunc
reorder_nodes_in_preorder_and_compute_unconnected_graph_order_if_component_is_non_trivial(
    ...
    const ShouldPlaceNodeAtTheEndOfTheOrder&
        should_place_node_at_the_end_of_the_order) {
  const int node_count = tail.image_count();
  const int arc_count = tail.preimage_count();
  ...
  // By reordering the nodes in preorder, we can guarentee, that the nodes of
  // every component are from a coninous range. As we sorted the arcs this is
  // also true for the arcs.

  // The following function is called for every component. The components are
  ...
  auto on_new_component = [&](int node_begin, int node_end, int arc_begin,
    ...
    auto sub_input_node_id =
        id_id_func(sub_node_count, input_node_id.image_count(),
    ...
    auto sub_order = compute_trivial_graph_order_if_graph_is_trivial(
        sub_tail, sub_head, sub_input_node_id, compute_connected_graph_order);
  ...
  // We identify components by marking the node in each component with the
  // minimum ID. We do this using the following observation, if an arc (u,v)
  // exists with u<v then v is not such a node

  BitIDFunc component_begin(node_count);
  component_begin.fill(true);
  for (int i = 0; i < arc_count; ++i)
    if (head(i) < tail(i)) component_begin.set(tail(i), false);
...
ArrayIDIDFunc compute_nested_dissection_graph_order(
  ...
  for (auto x : separator) in_separator.set(x, true);
...
ArrayIDIDFunc compute_nested_dissection_graph_order(
    ArrayIDIDFunc tail, ArrayIDIDFunc head, ArrayIDIDFunc input_node_id,
    const ComputeSeparator& compute_separator) {
  auto compute_graph_part_order = [&](ArrayIDIDFunc a_tail,
                                      ArrayIDIDFunc a_head,
  ...
  return compute_nested_dissection_graph_order(
      tail, head, input_node_id, compute_separator, compute_graph_part_order);
}

template <class ComputeCoreGraphOrder>
ArrayIDIDFunc
compute_graph_order_with_large_degree_three_independent_set_at_the_begin(
    ArrayIDIDFunc tail, ArrayIDIDFunc head, ArrayIDIDFunc input_node_id,
  ...
  auto degree = id_func(node_count, [&](int x) {
  ...
  for (auto x = 0; x < node_count; ++x) {
    if (degree(x) == 3) {
      ...
      if (!neighbor_in_set) in_independent_set.set(x, true);
    }
  }
  ...
  for (auto c = 0; c < node_count; ++c) {
    if (in_independent_set(c)) {
      ...
      auto cx = *iter++, cy = *iter++, cz = *iter++;
      assert(iter == inv_tail(c).end());
      auto xc = back_arc(cx), yc = back_arc(cy), zc = back_arc(cz);
      auto x = head(cx), y = head(cy), z = head(cz);
...
ArrayIDIDFunc compute_graph_order_with_degree_two_chain_at_the_begin(
  ...
  BitIDFunc node_in_core =
      id_func(node_count, [&](int x) { return degree(x) > 2; });

  for (auto first_arc = 0; first_arc < arc_count; ++first_arc) {
    auto chain_begin = tail(first_arc), chain_now = head(first_arc);
  ...
  auto order =
      reorder_nodes_in_preorder_and_compute_unconnected_graph_order_if_component_is_non_trivial(
          tail, head, input_node_id, compute_core_graph_order, node_in_core);

  {
    ArrayIDIDFunc super_to_sub(input_node_id.image_count(), node_count);
    for (int x = 0; x < node_count; ++x) super_to_sub[input_node_id(x)] = x;
    auto local_order = order;
    for (auto& x : local_order) x = super_to_sub[x];
    for (int p = 1; p < node_count; ++p) {
      auto x = local_order(p - 1), y = local_order(p);
      assert((!node_in_core(x) && !node_in_core(y)) ||
...
ArrayIDIDFunc compute_graph_order_with_largest_biconnected_component_at_the_end(
...

  {
    ...
    auto back_arc = compute_back_arc_permutation(tail, head);
    auto arc_component =
        compute_biconnected_components(out_arc, head, back_arc);
    auto largest_component =
        max_preimage_over_id_func(compute_histogram(arc_component));
  ...
  {
    BitIDFunc keep_flag = id_func(arc_count, [&](int a) {
      return node_in_largest_biconnected_component(tail(a)) ==
             node_in_largest_biconnected_component(head(a));
    });
  ...
  return reorder_nodes_in_preorder_and_compute_unconnected_graph_order_if_component_is_non_trivial(
      std::move(tail), std::move(head), std::move(input_node_id),
      compute_component_graph_order,
      std::move(node_in_largest_biconnected_component));
}

inline void make_graph_simple(ArrayIDIDFunc& tail, ArrayIDIDFunc& head) {
  const int node_count = tail.image_count();
  int arc_count = tail.preimage_count();
  (void)node_count;
  ...
  {
    BitIDFunc keep_flag = id_func(arc_count, [&](int i) -> bool {
      if (i != 0) {
        if (tail(i - 1) == tail(i) && head(i - 1) == head(i)) return false;
      }
...
ArrayIDIDFunc compute_nested_dissection_graph_order(
  ...
  auto compute_order = [&](ArrayIDIDFunc a_tail, ArrayIDIDFunc a_head,
                           ArrayIDIDFunc a_input_node_id) {
    return compute_nested_dissection_graph_order(
        std::move(a_tail), std::move(a_head), std::move(a_input_node_id),
        compute_separator);
...
ArrayIDIDFunc compute_cch_graph_order(
  ...
  auto orderer3 = [&](ArrayIDIDFunc a_tail, ArrayIDIDFunc a_head,
                      ArrayIDIDFunc a_input_node_id) {
    return compute_nested_dissection_graph_order(
        std::move(a_tail), std::move(a_head), std::move(a_input_node_id),
        compute_separator);
  ...
  auto orderer2 = [&](ArrayIDIDFunc a_tail, ArrayIDIDFunc a_head,
                      ArrayIDIDFunc a_input_node_id) {
  ...
  auto orderer1 = [&](ArrayIDIDFunc a_tail, ArrayIDIDFunc a_head,
                      ArrayIDIDFunc a_input_node_id) {
  ...
  auto order =
      reorder_nodes_in_preorder_and_compute_unconnected_graph_order_if_component_is_non_trivial(
          tail, head, input_node_id, orderer1, [](int) { return false; });
...
ArrayIDIDFunc compute_cch_graph_order(
    ArrayIDIDFunc tail, ArrayIDIDFunc head,
    const ComputeSeparator& compute_separator) {
  return compute_cch_graph_order(std::move(tail), std::move(head),
                                 identity_permutation(tail.image_count()),
                                 compute_separator);
}
}  // namespace cch_order
}  // namespace flowCutter
