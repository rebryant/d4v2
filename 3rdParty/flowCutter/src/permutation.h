...
bool is_permutation(const IDIDFunc& f) {
  if (f.preimage_count() != f.image_count()) return false;
  ...
  for (int i = 0; i < id_count; ++i) {
    int x = f(i);
    if (x < 0 || x >= id_count) return false;
    if (already_seen(x)) return false;
