  ...
  // Compute node levels
  {
    ...
    for (int i = 0; i < node_count; ++i) {
      ...
      if (children_count == 0) {
        ...
        int l = 1;
        while (!child_info.critical_list.empty() &&
               child_info.critical_list.back() == l) {
        ...
        ++tree_info.size;
        if (level.image_count() < l) level.set_image_count(l);
        level[x] = l - 1;
      } else {
        tree_info.size = 1;
        ...
        for (int i = 0; i <= p; ++i) crit.set(i, false);
        for (int i = p + 1; i <= max; ++i) {
          if (!crit(i)) {
            ...
            crit.set(i, false);
            if (q != 0) tree_info.critical_list.push_back(i);
          }
        }

        if (q == 0) {
          while (!first_child_critical_list.empty() &&
                 first_child_critical_list.back() <= max)
            first_child_critical_list.pop_back();
          q = max + 1;
          while (!first_child_critical_list.empty() &&
                 first_child_critical_list.back() == q) {
            first_child_critical_list.pop_back();
            ++q;
          }
        ...
        if (level.image_count() < q) level.set_image_count(q);
        level[x] = q - 1;
      }
