...
#define MAKE_TYPED_HAS_TRAIT(HAS, TYPE, EXPR)                             \
  ...
  template <class FT>                                                     \
  auto HAS##_impl1()                                                      \
      -> decltype(HAS##_impl2<FT>(static_cast<TYPE*>(nullptr)));          \
  template <class FT>                                                     \
...
MAKE_BOOL_TRAIT(is_id_id_func,
                has_preimage_count<FT>::value&& has_image_count<FT>::value&&
...
MAKE_BOOL_TRAIT(is_mutable_id_func,
                is_id_func<FT>::value&& has_set<FT>::value&&
                    has_fill<FT>::value&& has_move<FT>::value)
MAKE_BOOL_TRAIT(
    is_mutable_id_id_func,
    is_id_id_func<FT>::value&& has_set<FT>::value&& has_fill<FT>::value&&
        has_move<FT>::value&& has_set_image_count<FT>::value)
