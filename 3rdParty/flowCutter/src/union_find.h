...
struct UnionFind {
 public:
  ...
  explicit UnionFind(int node_count)
      : parent_(node_count),
        node_count_(node_count),
        component_count_(node_count) {
    parent_.fill(-1);
  }
  ...
  int operator()(int x) const {
    ...
    if (is_representative(x)) return x;

    int y = x;
    while (!is_representative(y)) y = parent_[y];
  ...
  bool in_same(int x, int y) const { return (*this)(x) == (*this)(y); }
